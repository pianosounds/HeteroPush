...
def get_nobalance_output():
    small_pred = np.load(weak_pred_path)
    large_pred = np.load(strong_pred_path)

    test_input = np.load('./output_winlen25_upload_results.npy')
    ...
    print()
    np.save('./defect_output_upload_results.npy',defect_input)
        


if __name__=='__main__':
    # upload_with_input()
    get_nobalance_output()